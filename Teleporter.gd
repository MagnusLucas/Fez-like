...
func check_position(delta : float) -> void:
	...
	var local_velocity : Vector3 = velocity * player.basis
	
	var global_x_velocity : Vector3 = local_velocity * Vector3(1, 0, 0) * player.basis.inverse()
	
	var current_cells := _get_inhabitet_cells_at_position(player.position)
	var future_cells := _get_inhabitet_cells_at_position(player.position + global_x_velocity * delta)
	var new_cells : Array[Vector3i] = Globals.subtract_arrays(future_cells, current_cells)
	
	var visibility_change : float
	var player_position : Vector3i
	if !new_cells.is_empty():
		visibility_change = _visibility_change(current_cells, future_cells)
		player_position = map.local_to_map(player.position)
	...
	current_cells = _get_inhabitet_cells_at_position(player.position + global_x_velocity * delta)
	future_cells = _get_inhabitet_cells_at_position(player.position + velocity * delta)
	new_cells = Globals.subtract_arrays(future_cells, current_cells)
...
func _handle_hiding(current_cells : Array[Vector3i],
	...
	match new_cells.size():
		...
		3:
			print_debug("There's 3 new cells, this should be impossible.")
	return
